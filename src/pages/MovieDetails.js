...
export const MovieDetails = () => {
  ...
  const imgURL = movie.poster_path ? `https://image.tmdb.org/t/p/w500/${movie.poster_path}` : BackupImage;

  useEffect(()=>{
    async function fetchMovie(params) {
      const response = await fetch(`https://api.themoviedb.org/3/movie/${paramsX.id}?api_key=b80d59c33d6d57ed9c7e3713f91c188a`);
  ...
  useEffect(()=> { document.title = `${movie.title} Cinemate` })
